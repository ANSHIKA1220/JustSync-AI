    ...
    <AppShell>
      ...
      <div className="grid gap-4 xl:grid-cols-[290px_1fr_360px]">
        ...
        <Card className="p-4">
          ...
          <h2 className="mt-5 font-semibold">Retrieved Sources</h2><div className="mt-2 space-y-2">{selected.ai_suggestion.sources.map((s: any, i: number) => <div key={s.chunk_id ?? i} className="rounded-md border border-slate-100 bg-slate-50 p-3 text-sm"><div className="mb-1 flex flex-wrap items-center gap-2"><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${s.provider === "chromadb" ? "bg-indigo-100 text-indigo-700" : "bg-slate-200 text-slate-600"}`}>{s.provider ?? "keyword"}</span><b className="flex-1 truncate">{s.title}</b><span className="shrink-0 text-xs font-medium text-slate-500">{s.similarity != null ? `${Math.round(s.similarity * 100)}%` : s.score != null ? `score ${s.score}` : ""}</span></div>{s.excerpt && <p className="line-clamp-2 text-xs text-slate-600">{s.excerpt}</p>}{s.retrieved_at && <p className="mt-1 text-[10px] text-slate-400">Retrieved {new Date(s.retrieved_at).toLocaleTimeString()}</p>}</div>)}</div>
