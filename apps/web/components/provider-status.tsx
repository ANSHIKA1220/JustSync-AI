...
export function AIProviderBadge({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => { api<HealthStatus>("/health").then(setStatus).catch(() => setStatus(null)); }, []);
  const tone = status?.fallback_active ? "amber" : status?.status === "healthy" ? "green" : "red";
  return <StatusBadge tone={tone} title={status?.fallback_active ? "Configured AI provider is unavailable; deterministic mock fallback is active." : "Current AI provider status."}>{compact ? providerLabel(status).replace("OpenAI-compatible", "OpenAI") : providerLabel(status)}</StatusBadge>;
}
...
export function SystemStatusPopover() {
  ...
  return (
    <div className="relative">
      <button aria-label="System status" onClick={() => setOpen(!open)} className={cn("focus-ring inline-flex h-9 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-sm", status?.fallback_active && "border-amber-300 bg-amber-50")}>
        <Activity className="size-4 text-accent" /> Status
      </button>
      {open && <Card className="absolute right-0 top-11 z-30 w-80 p-4">
        <div className="flex items-center justify-between"><h3 className="font-semibold">System Status</h3><Button className="h-8 px-2" onClick={refresh}><RefreshCw className="size-3" /> Refresh</Button></div>
        <div className="mt-4 space-y-3 text-sm">
          ...
          <div className="flex items-center justify-between"><span>Polling</span><StatusBadge tone="green">5s active</StatusBadge></div>
          <p className="text-xs text-slate-500">Last health check: {checkedAt || "not checked"}</p>
        </div>
      </Card>}
    </div>
  );
